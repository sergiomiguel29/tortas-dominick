...
export default function Builder(){
 ...
 function select(value:string){
   setChoices(previous=>({...previous,[current.key]:value}));
 ...
 return <><SubHeader active="Arma tu torta"/><main className="builder-page">
  ...
  <div className="builder-shell">
   ...
   <section className={`builder-summary ${summaryOpen?"summary-open":""}`} ref={summaryRef}><p>TU CREACIÓN</p><h2>Completa tu pedido</h2><button className="summary-toggle" onClick={()=>setSummaryOpen(!summaryOpen)} aria-expanded={summaryOpen}>{summaryOpen?"Ocultar resumen":"Ver resumen"} ({Object.keys(choices).length}) <span>{summaryOpen?"−":"+"}</span></button><div className="summary-collapsible"><div className="builder-preview"><div className="crop-c"><img src="/images/dominick-3.jpg" alt="Detalle Dominick"/></div><div className="crop-a"><img src="/images/dominick-2.jpg" alt="Torta Dominick"/></div><div className="crop-b"><img src="/images/dominick-3.jpg" alt="Acabado Dominick"/></div></div>{steps.map(s=><div className="summary-row" key={s.key}><span>{s.title.replace("Elige el ","").replace("Escoge el ","").replace("Selecciona el ","").replace("Define el ","")}</span><b>{choices[s.key]||"Por elegir"}</b></div>)}</div><label className="order-field"><span className="field-label">Nombre</span><input type="text" autoComplete="name" value={name} onChange={e=>setName(e.target.value)} placeholder="Escribe tu nombre"/></label><label className="order-field"><span className="field-label">Ocasión</span><select value={occasion} onChange={e=>setOccasion(e.target.value)}><option value="">Selecciona una ocasión</option><option>Cumpleaños</option><option>Boda</option><option>Aniversario</option><option>Evento infantil</option><option>Otra celebración</option></select></label><label className="order-field date-field"><span className="field-label">Fecha deseada</span><span className="field-help" id="date-help">Selecciona el día en que necesitas tu pedido</span><span className="date-control"><input aria-describedby="date-help" type="date" value={date} onChange={e=>setDate(e.target.value)}/><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3v3M17 3v3M4 9h16M5 5h14a1 1 0 0 1 1 1v14H4V6a1 1 0 0 1 1-1Z"/><path d="M8 13h3v3H8z"/></svg></span></label><label className="order-field"><span className="field-label">Detalles adicionales</span><span className="field-help">Colores, texto para la torta o temática</span><textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Ej.: tonos rosados, nombre Camila, flores..."/></label>{complete?<WhatsAppLink message={message} className="builder-send">Enviar mi diseño a {branch?.name??"mi sucursal"} →</WhatsAppLink>:<button className="builder-send disabled" disabled>Completa los 4 pasos</button>}<small className="builder-note">Luego podrás confirmar precio y disponibilidad por WhatsApp.</small></section>
  </div>
 </main></>;
}
